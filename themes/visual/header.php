...
<html lang="<?= $App->get( 'lang' ) ?>">
  <head>
    ...
    <link rel="stylesheet preload" as="style" href="https://fonts.googleapis.com/css2?family=Didact+Gothic&display=swap">
    <link rel="stylesheet preload" as="style" href="<?= $App->theme( 'style.css', false ) ?>">
    ...
    <div class="ss-top ss-bg-light-black ss-white ss-py-5">
      <div class="ss-container ss-center ss-large ss-my-5">
        <?php foreach ( $App->get( 'vsl' )[ 'menu' ] as $menu ): ?>
          ...
          <a href="<?= $App->esc( $menu[ 'link' ] ) ?>" class="ss-button ss-card-2 ss-bg-transparent"><?= $menu[ 'text' ] ?></a>
          <?php endif ?>
        <?php endforeach ?>
      </div>
      <?php if ( vsl_is_home() ): ?>
      <div class="ss-container ss-my-7">
        <h1 class="subtitle ss-wide ss-mobile ss-w-8 ss-mx-auto"><?= $App->get( 'vsl' )[ 'top' ] ?></h1>
      </div>
      <?php $App->get_action( 'site_under_subtitle' ) ?>
      <?php endif ?>
    </div>
    <div class="ss-container ss-mobile ss-w-8 ss-my-5 ss-mx-auto ss-py-7">
      <div class="ss-container ss-large content-text">
